...
const feriados = [
  ...
  new Date(2025,3,18),
  new Date(2025,4,1),
  new Date(2025,4,2),   // puente
  new Date(2025,4,25),
  new Date(2025,5,16),  // Güemes trasladado
  new Date(2025,5,20),
  new Date(2025,6,9),
  new Date(2025,7,15),  // San Martín trasladado
  new Date(2025,9,12),  // Diversidad cultural (cae domingo, lo incluimos igual)
  new Date(2025,10,21), // puente
  new Date(2025,10,24),
  new Date(2025,11,8),
  new Date(2025,11,25)
];
...
function validarPaso1() {
  ...
  asae = document.getElementById("asae").value;       // "1".."5", "2d", etc.
  ambiente = document.getElementById("ambiente").value; // "exterior" | "interior"
  ...
  es_feriado = feriados.some(d => mismoDia(fecha, d));
...
function calcularRuido() {
  ...
  if (habilitaLF7 && msiete === "S") {
    const lfmsiete = lf + 7;
    const lmpFijo = lmp;
    const lmpAplicable = Math.max(lmpFijo, lfmsiete);
    ...
    document.getElementById('lmplblm7d').textContent = "LMP:";
